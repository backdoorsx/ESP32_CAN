/*
  https://www.waveshare.com/wiki/ESP32-S3-LCD-2
...
void set_unit(){
    ...
    //tft.setTextSize(2); //4
    tft.setCursor(180, tft.height() - 128);  // (180, tft.height() - 40)
    tft.println("KPa");
}
...
void erase_value() {
    tft.fillRect(0, tft.height()/2-22, tft.width()/2+16, 72, TFT_BLACK);
}
...
int simulateMAP() {
  ...
  // 2-4s -> 35 ±2
  if (t < 4000) {
    return 35 + random(-2, 3);
  }

  // 4-10s -> 35 -> 250
  if (t < 10000) {
    float p = (t - 4000) / 6000.0;
    return 35 + (250 - 35) * p;
  }
  ...
  // rýchly pokles 270 -> 50
  if (cycle < 1000) {
    float p = (cycle - 500) / 500.0;
    return 270 - (270 - 50) * p;
  }

  // rast 50 -> 270
  float p = (cycle - 1000) / 2000.0;
  return 50 + (270 - 50) * p;
}