...
void loop() {
  ...
  static uint32_t send_timer_map = millis();
  ...
  while (true) {
    ...
    if (twai_receive(&message, pdMS_TO_TICKS(10)) == ESP_OK) {
      ...
      if (bufferIndex < BUFFER_SIZE) {
        ...
        // ak je ID 0x00000424, uloz data[2]
        //if (message.identifier == 0x00000424 && dlc > 2) {
        //  value_424 = message.data[2];
        //}
        if (message.identifier == 0x000007E8 && dlc >= 4) {
        // 0x41 = OBD response
        // 0x0B = MAP PID
          if (message.data[1] == 0x41 && message.data[2] == 0x0B) {
            value_424 = message.data[3];  // MAP v kPa
          }
        }
        // -----------------------------------------------
    ...
    if (millis() - send_timer_map >= 150) {
      send_obd_map_request();
      send_timer_map = millis();
    }
